...
class SearchDrinksContainingIngredientsRemoteMediator (
    ...
    override suspend fun load(loadType: LoadType, state: PagingState<Int, Drink>): MediatorResult {
        ...
        return try {
            val drinkResponses = drinkApi.searchDrinksByIngredients(ingredientFamilyName = query)
            ...
            if (!drinkResponses.isNullOrEmpty()){
                drinkResponses.forEach { drinkResponse ->
                    drinkDatabase.withTransaction {
                        ...
                        val drinkRemoteKeys = drinkResponse.drinks.map { drink ->
                            DrinkRemoteKeys(
                                id = drink.drinkId,
                        ...
                        drinkRemoteKeysDao.addAllDrinkRemoteKeys(drinkRemoteKeys = drinkRemoteKeys)
